...
void loop() {
    ...
    if(rc.rc_in.AUX > 1500) {
        ...
        float roll = map(rc.rc_in.AIL, MIN_PWM_OUT, MAX_PWM_OUT, -1.0, 1.0);
        float pitch = map(rc.rc_in.ELE, MIN_PWM_OUT, MAX_PWM_OUT, -1.0, 1.0);
        float yaw = map(rc.rc_in.RUD, MIN_PWM_OUT, MAX_PWM_OUT, -1.0, 1.0);
